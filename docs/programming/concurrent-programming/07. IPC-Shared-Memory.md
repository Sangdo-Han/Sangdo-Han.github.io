...
sequenceDiagram
    ...
    participant Mutex
    participant Condition Variable
    participant Thread 1
    participant Thread 2
    participant Atomic Boolean (Stop Flag)

    Client 1->>Task Queue: Submit Task A (Lock Mutex)
    Client 2->>Task Queue: Submit Task B (Lock Mutex)
    Task Queue-->>Mutex: Unlock After Insertion
    Task Queue-->>Condition Variable: Notify Worker Threads
    
    Condition Variable-->>Thread 1: Wake Up
    Thread 1->>Task Queue: Fetch Task A (Lock Mutex)
    Task Queue-->>Mutex: Unlock After Fetch
    Thread 1->>Thread 1: Process Task A

    Condition Variable-->>Thread 2: Wake Up
    Thread 2->>Task Queue: Fetch Task B (Lock Mutex)
    Task Queue-->>Mutex: Unlock After Fetch
    Thread 2->>Thread 2: Process Task B

    Atomic Boolean (Stop Flag)->>Thread 1: Check Termination Condition
    Atomic Boolean (Stop Flag)->>Thread 2: Check Termination Condition
