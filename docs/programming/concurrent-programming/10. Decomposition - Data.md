            ...
            mWorkers.emplace_back([this] {
                while (true) {
                    std::function<void()> task;
                    {
                        ...
                        mConditionVariable.wait(lk, [this] { return mStop || !mTasks.empty(); });
    ...
    void Enqueue(Func&& f, Args&&... args) {
        {
            ...
            mTasks.emplace(std::bind(std::forward<Func>(f), std::forward<Args>(args)...));
        }
...
int main() {
    ...
    std::uniform_int_distribution<> distrib(1, numCandidates);

    std::generate(pile.begin(), pile.end(), [&]() { return distrib(gen); });
