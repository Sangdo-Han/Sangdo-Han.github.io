...
We've only discussed basic syntax, memory related features ... What's next?       

Crates (to use external sources and to make a bigger program), standard libraries (I/O, threading, ... ) and many other things might needs to be treated.

It is better to understand those subjects with actual projects, not like the subjects we dealt with. 

Therefore, I will suggest some project in the future to work and learn with Rust Programming.

However, since I do my program mainly with C++ and python, future post will be postponed.
