...
&nbsp; Now we can discuss control of LLM with the paper [[1](#Bhargava-Aman-et-al)].    
### Notations
...
### Assumptions
&nbsp; Now, let us follow the defininition of LLM system: 

1. It is natural that the LLM system is discrete: as we define the token be a state, time would be determined by the sequence.

2. It is assumed that the LLM system follows the Shift-and-Grow State Dynamics: Whereas the system state in an ODE-based system has a fixed size over time, the system state x(t) for LLM systems grows as tokens are added to the state sequence.

3. It is assumed that the LLM system follows a Markov Transition (In paper it is said in more gently as `Mutual exclusion on control input token vs. generated token`): The newest token is either drawn from the control input $$u(t)$$ or is generated by the LLM by sampling $$x{'} \sim  P_{LM} (x' \| x(t))$$. This differs from traditional discrete stochastic systems, where the control sequence and internal dynamics generally affect the state synchronously.

&nbsp; With the basic definition of control system and the followed assumptions, the authors defined the LLM systems and Control Input as follows:

- $$ t \in T = N$$ : time $$t$$ is in natural number set.
